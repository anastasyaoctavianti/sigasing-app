          ...
          while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
            ...
            <tr>
              ...
              <td>
                ...
                ?>
                <a href="?page=karyawanbagian&id=<?php echo $row['id'] ?>" class="btn bg-cyan btn-sm mr-1"><i class="fa fa-building">&nbsp;</i><?php echo $bagian_terkini ?></a>
              </td>
              <td>
                <?php
                $jabatan_terkini = $row['jabatan_terkini'] == "" ? "Belum ada" : $row['jabatan_terkini'];
                ?>
                <a href="?page=karyawanjabatan&id=<?php echo $row['id'] ?>" class="btn bg-green btn-sm mr-1"><i class="fa fa-user">&nbsp;</i><?php echo $jabatan_terkini ?></a>
              </td>
              <td>
                <a href="?page=karyawanupdate&id=<?php echo $row['id'] ?>" class="btn btn-primary btn-sm mr-1">
                  <i class="fa fa-edit"></i> Ubah
                </a>
                <a href="?page=karyawandelete&id=<?php echo $row['id']. "&pengguna=".$row['pengguna_id'].""?>" class="btn btn-danger btn-sm"
